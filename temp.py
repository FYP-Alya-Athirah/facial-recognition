        ...
        if process_this_frame:
            ...
            for face_encoding in face_encodings:
                ...
                if matches[best_match_index]:
                    ...
                    logger.info("Face detected: %s",name)
                    ...
                    # Step 2: Check if day is morning(attendance 5:00am - 11:00am) or evening(dismissal 11:00am -8:00pm)
                    if time_in_range(datetime.time(5,0,0), datetime.time(12,0,0), now.time()):
                    ...
                    elif time_in_range(datetime.time(11, 0, 0), datetime.time(20, 0, 0), now.time()):
                        if(role == 2):
...
def set_attendance_student(name, student_id, formatted_date, formatted_time):
    ...
    mycursor.execute(sql, val)
    mydb.commit()
    logger.info("Attendance updated in database for: %s", name)
...
def set_dismissal_student_parent(name, student_id, formatted_date, formatted_time, parent):
    ...
    mycursor.execute(sql, val)
    mydb.commit()
    logger.info("Dismissal updated in database for: %s", name)
...
# while True:
#     time.sleep(3)
#     logger.info(directory_update)
#     if (directory_update):
#         t1.join()
#         logger.info("Thread 1 joined")
#         t1 = threading.Thread(target=gen_frames, name='t1')
#         t1.start()
#         directory_update = False
